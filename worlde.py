...
regexexp = openbracket + listOfLists[0] + closebracket + openbracket + listOfLists[1] + closebracket + openbracket + \
    listOfLists[2] + closebracket + openbracket + listOfLists[3] + \
    closebracket + openbracket + listOfLists[4] + closebracket + dollar
...
while len(possibleSolution) > 1:
    ...
    for i in range(0, len(val)):
        ...
        if pos == 1:  # The character exists in the answer at the correct position
            ...
            flag = True
            for j in range(0, len(val)): # Check if the same character is entered before, in that case it is NO OP
                if val[i] == val[j] and i != j:
                    flag = False
                    break
